...
def generate_ruptures(home,project_name,run_name,fault_name,slab_name,mesh_name,
		load_distances,distances_name,UTM_zone,realizations,model_name,hurst,
        Ldip, Lstrike,num_modes,rake,rise_time_depths,time_epi,
		max_slip,source_time_function,lognormal,slip_standard_deviation,
        scaling_law,ncpus,
		force_magnitude=False,force_area=False,
        mean_slip_name=None,hypocenter=None,
		slip_tol=1e-2,force_hypocenter=False,
        no_random=False, shypo=None,use_hypo_fraction=True,
    ...
    # Make a list of arguments needed for generate_one_rupture.
    # Only the first argument realization changes as we loop through all the 
    # ruptures. (Which will be done in parallel with multiprocessing.Pool.)

    all_args = []
    for realization in realizations:
        args = (realization,Dstrike,Ddip,vel_mod_file,velmod,whole_fault,
                home,project_name,run_name,fault_name,slab_name,mesh_name,
        		load_distances,distances_name,UTM_zone,model_name,hurst,
                Ldip, Lstrike,num_modes,rake,rise_time_depths,time_epi,
        		max_slip,source_time_function,lognormal,slip_standard_deviation,
                scaling_law,
        		force_magnitude,force_area,
                mean_slip_name,hypocenter,
        		slip_tol,force_hypocenter,
    ...
    # the arguments needed for generate_one_rupture.  
    # Farm these out to the requested number of processes, specified by ncpus:
    
    ncpus_to_use = min(ncpus, len(realizations))  # reduce if fewer ruptures
    
    print('Distributing %i realizations among %i processes' \
            % (len(realizations),ncpus_to_use))

    with Pool(processes=ncpus_to_use) as pool:
        pool.starmap(generate_one_rupture, all_args)
    ...
    print('Ruptures are in %s' % home+project_name+'/output/ruptures')
    
    # end of generate_ruptures
    

def generate_one_rupture(realization,Dstrike,Ddip,vel_mod_file,velmod,whole_fault,
        home,project_name,run_name,fault_name,slab_name,mesh_name,
		load_distances,distances_name,UTM_zone,model_name,hurst,
        Ldip, Lstrike,num_modes,rake,rise_time_depths,time_epi,
		max_slip,source_time_function,lognormal,slip_standard_deviation,
    ...
    print('+++ Process %i creating fakequake rupture for realization %s' \
            % (current_process().pid, realization.rname))
    ...
    while success==False:
        ...
        if lognormal==False:
            ...
            C_nonlog=fakequakes.get_covariance(mean_slip,C,current_target_Mw,
                    fault_array,vel_mod_file,slip_standard_deviation) 
            #Get eigen values and eigenvectors
            eigenvals,V=fakequakes.get_eigen(C_nonlog)
            #Generate fake slip pattern
            rejected=True
            while rejected==True:
                slip_unrectified,success=fakequakes.make_KL_slip(fault_array,
                    num_modes,eigenvals,V,mean_slip,max_slip,
                    lognormal=False,seed=12345)
                slip,rejected,percent_negative=fakequakes.rectify_slip(slip_unrectified,
                    percent_reject=13)
                if rejected==True:
                    print('... ... ... negative slip threshold exceeeded with %d%% negative slip. Recomputing...' % (percent_negative))
        else:
            #Get lognormal values
            C_log,mean_slip_log=fakequakes.get_lognormal(mean_slip,C,
                    current_target_Mw,fault_array,vel_mod_file,
                    slip_standard_deviation)               
            #Get eigen values and eigenvectors
            eigenvals,V=fakequakes.get_eigen(C_log)
            #Generate fake slip pattern
            slip,success=fakequakes.make_KL_slip(fault_array,num_modes,
                    eigenvals,V,mean_slip_log,max_slip,
                    lognormal=True,seed=12345)
